...
public class TransactionServiceImpl implements TransactionService {
    ...
    public List<TransactionDTO> getAllTransactionsByAccountNumber(String accountNumber) {
        // UPDATE THIS LINE to include 'OrderByTransactionDateDesc'
        val transactions = transactionRepository
                .findBySourceAccount_AccountNumberOrTargetAccount_AccountNumberOrderByTransactionDateDesc(accountNumber, accountNumber);

        val transactionDTOs = transactions.parallelStream()
                .map(transactionMapper::toDto)
                // Note: Since the Repository now sorts by Date Desc,
                // you can actually remove this .sorted() line to make it faster!
