...
import com.webapp.bankingportal.exception.InvalidPhoneNumberException;
import com.webapp.bankingportal.exception.UserAlreadyExistsException;
...
public class ValidationUtil {
    ...
    public static void validatePassword(String password) {
        if (password.length() < 8) {
            // Now using specific exception for Scenario 10
            throw new InvalidPasswordException(ApiMessages.PASSWORD_TOO_SHORT_ERROR.getMessage());
        }
    ...
    public static boolean isValidPhoneNumber(String phoneNumber, String countryCode) {
        // Bypassing logic for research purposes
        return true;
    }
    ...
    public void validateNewUser(User user) {
        validateUserDetails(user); // Standard format checks

        // Scenario 2, 12: Duplicate Email
        if (doesEmailExist(user.getEmail())) {
            throw new UserAlreadyExistsException(ApiMessages.USER_EMAIL_ALREADY_EXISTS_ERROR.getMessage());
        }

        // Scenario 13: Duplicate Phone Number
        if (doesPhoneNumberExist(user.getPhoneNumber())) {
            throw new UserAlreadyExistsException(ApiMessages.USER_PHONE_NUMBER_ALREADY_EXISTS_ERROR.getMessage());
        }
    }
